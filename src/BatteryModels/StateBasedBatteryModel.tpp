...
StateBasedBatteryModel<ECMStateEstimator, VoltageInterpolator, CurrentInterpolator, SocToOcv,
                       OcvToSoc, SocEstimator, CapacityEstimator,
                       OcvEstimator>::StateBasedBatteryModel(double capacity,
                                                             Eigen::VectorXd &params,
                                                             bool useMeasuredCapacity)
    : useMeasuredCapacity(useMeasuredCapacity),
      deltaTimeMult(std::pow(
          10, params[SocEstimator::getParamsCount() + ECMStateEstimator::getParamsCount() +
                     VoltageInterpolator::getParamsCount() + CurrentInterpolator::getParamsCount() +
                     SocToOcv::getParamsCount() + OcvToSoc::getParamsCount() +
                     CapacityEstimator::getParamsCount() + OcvEstimator::getParamsCount()])),
      socCountThreshold(int(std::pow(
          10, params[SocEstimator::getParamsCount() + ECMStateEstimator::getParamsCount() +
                     VoltageInterpolator::getParamsCount() + CurrentInterpolator::getParamsCount() +
                     SocToOcv::getParamsCount() + OcvToSoc::getParamsCount() +
                     CapacityEstimator::getParamsCount() + OcvEstimator::getParamsCount() + 1]))),
      maxTimeDiffMult(std::pow(
          10, params[SocEstimator::getParamsCount() + ECMStateEstimator::getParamsCount() +
                     VoltageInterpolator::getParamsCount() + CurrentInterpolator::getParamsCount() +
                     SocToOcv::getParamsCount() + OcvToSoc::getParamsCount() +
                     CapacityEstimator::getParamsCount() + OcvEstimator::getParamsCount() + 2])),
      initialCapacity(capacity), deltaTime(0), socEstimatorSetupEcm(false),
      socEstimatorSetupOcv(false), countSinceOcv(1000), totalVoltageErrorSq(0),
      voltagePredictionCount(0), totalCapacityErrorSq(0), capacityPredictionCount(0),
      resistancePredictionCount(0), missedCapacityPredictionCount(0),
      ecmStateEstimator(
          params.segment(SocEstimator::getParamsCount(), ECMStateEstimator::getParamsCount())),
      voltageInterpolator(
          params.segment(SocEstimator::getParamsCount() + ECMStateEstimator::getParamsCount(),
      ...
      socToOcv(ReadOcvSocCSV("/mnt/c/Users/Danie/Desktop/project/data/soc_volt_nasa.csv"),
               params.segment(SocEstimator::getParamsCount() + ECMStateEstimator::getParamsCount() +
                                  VoltageInterpolator::getParamsCount() +
                                  CurrentInterpolator::getParamsCount(),
                              SocToOcv::getParamsCount())),
      ...
      ocvEstimator(
          params.segment(SocEstimator::getParamsCount() + ECMStateEstimator::getParamsCount() +
                             VoltageInterpolator::getParamsCount() +
                             CurrentInterpolator::getParamsCount() + SocToOcv::getParamsCount() +
                             OcvToSoc::getParamsCount() + CapacityEstimator::getParamsCount(),
                         OcvEstimator::getParamsCount())),
      socEstimator(ECMStateEstimator::getDimension(), &socToOcv, &ocvToSoc,
                   params.segment(0, SocEstimator::getParamsCount())) {
    // Constructor body is mostly empty - most initialization done in initializer list
...
size_t
StateBasedBatteryModel<ECMStateEstimator, VoltageInterpolator, CurrentInterpolator, SocToOcv,
                       OcvToSoc, SocEstimator, CapacityEstimator, OcvEstimator>::getParamsCount() {
    return SocEstimator::getParamsCount() + ECMStateEstimator::getParamsCount() +
           VoltageInterpolator::getParamsCount() + CurrentInterpolator::getParamsCount() +
           SocToOcv::getParamsCount() + OcvToSoc::getParamsCount() +
           CapacityEstimator::getParamsCount() + OcvEstimator::getParamsCount() + 3;
    // Extra parameters:
    // deltaTimeMult
    // socCountThreshold
    // maxTimeDiffMult
...
Eigen::VectorXd
StateBasedBatteryModel<ECMStateEstimator, VoltageInterpolator, CurrentInterpolator, SocToOcv,
                       OcvToSoc, SocEstimator, CapacityEstimator, OcvEstimator>::getLowerBounds() {
    ...
    if (ECMStateEstimator::getParamsCount() > 0) {
        v.segment(offset, ECMStateEstimator::getParamsCount()) =
    ...
    if (SocToOcv::getParamsCount() > 0) {
        v.segment(offset, SocToOcv::getParamsCount()) = SocToOcv::getLowerBounds();
        offset += SocToOcv::getParamsCount();
    }

    if (OcvToSoc::getParamsCount() > 0) {
        v.segment(offset, SocToOcv::getParamsCount()) = OcvToSoc::getLowerBounds();
...
Eigen::VectorXd
StateBasedBatteryModel<ECMStateEstimator, VoltageInterpolator, CurrentInterpolator, SocToOcv,
                       OcvToSoc, SocEstimator, CapacityEstimator, OcvEstimator>::getUpperBounds() {
    Eigen::VectorXd v(getParamsCount());
    ...
    if (SocToOcv::getParamsCount() > 0) {
        v.segment(offset, SocToOcv::getParamsCount()) = SocToOcv::getUpperBounds();
        offset += SocToOcv::getParamsCount();
    }

    if (OcvToSoc::getParamsCount() > 0) {
        v.segment(offset, SocToOcv::getParamsCount()) = OcvToSoc::getUpperBounds();
        offset += OcvToSoc::getParamsCount();
    }
...
template <typename ECMStateEstimator, typename VoltageInterpolator, typename CurrentInterpolator,
          typename SocToOcv, typename OcvToSoc, typename SocEstimator, typename CapacityEstimator,
          typename OcvEstimator>
void StateBasedBatteryModel<ECMStateEstimator, VoltageInterpolator, CurrentInterpolator, SocToOcv,
                            OcvToSoc, SocEstimator, CapacityEstimator,
    ...
    for (int i = 0; i < n; i++) {
        ...
        if ((countSinceOcv > socCountThreshold || socEstimatorSetupOcv) &&
            ocvEstimator.canCalculateState()) {
            countSinceOcv = 0;
        ...
        if (socEstimatorSetupEcm && socEstimatorSetupOcv && i > 0) {
            ...
            if (socEstCount > 10) { // ignore first 10
                ...
                const double voltageChange =
                    voltages[i] -
                    voltages[i -
                             1]; // use this instead so I can check aginst just using last voltage.
...
void StateBasedBatteryModel<ECMStateEstimator, VoltageInterpolator, CurrentInterpolator, SocToOcv,
    ...
    if (missedCapacityPredictionCount > 5 && std::abs(predCapacity - initialCapacity) > 1e-1) {
...
template <typename ECMStateEstimator, typename VoltageInterpolator, typename CurrentInterpolator,
          typename SocToOcv, typename OcvToSoc, typename SocEstimator, typename CapacityEstimator,
          typename OcvEstimator>
double StateBasedBatteryModel<ECMStateEstimator, VoltageInterpolator, CurrentInterpolator, SocToOcv,
                              OcvToSoc, SocEstimator, CapacityEstimator,
        ...
        case ErrorMetric::ResistanceError:
            if (resistancePredictionCount > 0) {
                return totalResistanceErrorSq / resistancePredictionCount *
                       (1.0 +
                        0.5 * double(missedResistancePredictionCount) /
                            double(missedResistancePredictionCount + resistancePredictionCount));
            } else {
                return 1e100;
            }
        case ErrorMetric::VoltageError:
            if (voltagePredictionCount > 0) {
                return totalVoltageErrorSq / voltagePredictionCount *
                       (1.0 + 0.5 * double(missedVoltagePredictionCount) /
                                  double(missedVoltagePredictionCount + voltagePredictionCount));
            } else {
                return 1e100;
            }
            ...
            if (capacityPredictionCount > 0) {
                return totalCapacityErrorSq / capacityPredictionCount *
                       (1.0 + 0.5 * double(missedCapacityPredictionCount) /
                                  double(missedCapacityPredictionCount + totalCapacityVariance));
    ...
    switch (metric) {
        case ErrorMetric::ResistanceError:
            std::cout << "Resistance \tMSE " << getObjectiveValue(metric) << "\n"
                      << "\t 1-R^2 "
                      << (totalResistanceErrorSq / resistancePredictionCount) /
                             (totalResistanceVariance / (resistancePredictionCount - 1))
                      << "\n"
        ...
        case ErrorMetric::VoltageError:
            std::cout << "Voltage \tMSE " << getObjectiveValue(metric) << "\n"
                      << "\t 1-R^2 "
                      << (totalVoltageErrorSq / voltagePredictionCount) /
                             (totalVoltageVariance / (voltagePredictionCount - 1))
                      << "\n"
