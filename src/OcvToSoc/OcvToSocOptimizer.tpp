...
template <typename T> OcvToSocOptimizer<T>::OcvToSocOptimizer() {
  OcvSocData data = ReadOcvSocCSV(
      "/mnt/c/Users/Danie/Desktop/project/data/soc_volt_nasa.csv");
  ...
  for (int i = 0; i < n; ++i) {
    ...
    if (mod_result <
        (DATA_SPLIT - 2)) { // Train set (positions 0, 1, 2, 3 mod 6)
...
template <typename T>
T *OcvToSocOptimizer<T>::makeModel(const OcvSocData &data,
...
template <typename T>
void OcvToSocOptimizer<T>::display(const Eigen::VectorXd &params) {
  T *model = makeModel(train, params);
  ...
  for (int i = 0; i < n; i++) {
    double pred_soc =
        model->getSoc(test.ocv(i)); // Fixed: use test.soc, added (i) and ->
    MSE +=
        (pred_ocv - test.soc(i)) * (pred_soc - test.soc(i)); // Fixed: added (i)
  ...
  double R2_leftover =
      MSE / variance; // because easier to read when gets very close to 1.
...
public:
  BayesOptObjective(OcvToSocOptimizer<T> *optimizer)
      : ContinuousModel(T::getParamsCount(), bayesopt::Parameters()),
  ...
  double evaluateSample(const vectord &x) override {
    ...
    for (size_t i = 0; i < x.size(); ++i) {
      params(i) =
          x[i] * (upper_bounds_[i] - lower_bounds_[i]) + lower_bounds_[i];
    }
...
template <typename T> void OcvToSocOptimizer<T>::optimize() {
  ...
  for (int i = 0; i < T::getParamsCount(); ++i) {
    optimal_params(i) =
        result[i] * (upper_bounds[i] - lower_bounds[i]) + lower_bounds[i];
  }
