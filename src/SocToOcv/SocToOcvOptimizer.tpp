...
template<typename T>
T* SocToOcvOptimizer<T>::makeModel(const OcvSocData& data, const Eigen::VectorXd& params) {
    return new T(data, params);
}

template<typename T>
double SocToOcvOptimizer<T>::getObjectiveValue(const Eigen::VectorXd& params) {
...
template<typename T>
void SocToOcvOptimizer<T>::display(const Eigen::VectorXd& params) {
    T* model = makeModel(train, params);
...
public:
    BayesOptObjective(SocToOcvOptimizer<T>* optimizer) : 
...
private:
    SocToOcvOptimizer<T>* optimizer_;
    Eigen::VectorXd lower_bounds_;
    Eigen::VectorXd upper_bounds_;
};

template<typename T>
void SocToOcvOptimizer<T>::optimize() {
