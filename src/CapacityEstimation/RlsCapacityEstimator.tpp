...
RlsCapacityEstimator<RLS>::RlsCapacityEstimator(const double capacity,
...
template <typename RLS> void RlsCapacityEstimator<RLS>::update(double currentArea, double diffSoc) {
    Eigen::VectorXd x(2);
    x(0) = 1;
    x(1) = -currentArea;
...
template <typename RLS> double const RlsCapacityEstimator<RLS>::getCapacity() {
...
template <typename RLS> double const RlsCapacityEstimator<RLS>::getCapacityVariance() {
    const double slope = std::abs(model.getState()(1));
    if (slope < 1e-5)
        return 1e100;
    const double var = std::abs(model.getCovariance()(1, 1));
    // order 3 delta method from
...
template <typename RLS> bool const RlsCapacityEstimator<RLS>::canCalculateStateCapacity() {
    if (std::abs(model.getState()(1)) < 1e-5)
        return false;
...
template <typename RLS> size_t RlsCapacityEstimator<RLS>::getParamsCount() {
    return RLS::getParamsCount() + 1;
}

template <typename RLS> const Eigen::VectorXd RlsCapacityEstimator<RLS>::getLowerBounds() {
...
template <typename RLS> const Eigen::VectorXd RlsCapacityEstimator<RLS>::getUpperBounds() {
    Eigen::VectorXd v(getParamsCount());
